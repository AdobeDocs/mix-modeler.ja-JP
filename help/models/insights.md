...
>abstract="このビジュアライゼーションでは、すべてのコンバージョンパスをまたいだタッチポイントとタッチポイントの位置別に、アトリビューションコンバージョンの分類を表示します。 ビジュアライゼーションでは、タッチポイントの貢献度が、残りの位置や他の任意の位置のタッチポイントの貢献度よりも高いかどうかを比較します。"
...
>title="モデルの日付（日時）"
>abstract="このテーブルのデータは、特定の期間のみ生成されます。**[!UICONTROL As of]** 日付は、データが生成された日付を示し、指定された日付から 1 年前までのデータに基づいています。"
