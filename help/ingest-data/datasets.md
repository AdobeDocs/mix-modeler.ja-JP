---
title: データセット
description: Mix Modelerにデータを取り込むために必要なデータセットを管理する方法について説明します。
feature: Datasets
exl-id: 788de24d-9021-48f8-87ab-09252b91276a
TQID: https://experienceleague.adobe.com/gcb3YrQjRpQfKspkh6T-WZdM-2HosqDNb3otqym-cBk
product_v2:
  - id: b88c80e3-31df-4609-989d-d4dac0e6d973
feature_v2:
  - id: e0abf868-dae2-4c1c-83e9-b21799232845
subfeature_v2:
  - id: ad7101f7-ae92-401b-a25a-d3060d42989d
  - id: a4dc3e7d-bd07-4ac8-8e49-ff2e8fecf1e7
  - id: ee1bf083-e090-4def-936b-c111d29f42d0
  - id: d1167c89-f64a-42ca-ac95-1d91b7790df2
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
  - id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
autotag-review: '2026-05-01T09:11:08.694Z'
source-git-commit: 5579087b9381c4d8e909ed5fe3099fd42d5c6799
workflow-type: tm+mt
source-wordcount: 97
ht-degree: 19%

---

# データセット

Experience Platformに取り込み、Mix Modelerで使用するデータを含むデータセットを管理するには：

1. Mix Modelerのインターフェイスに移動します。

1. **[!UICONTROL SETUP]**&#x200B;の下にある![Data](/help/assets/icons/Data.svg) **[!UICONTROL Datasets]**&#x200B;を選択します。

詳しくは、[&#x200B; データセット UI ガイド &#x200B;](https://experienceleague.adobe.com/docs/experience-platform/catalog/datasets/user-guide.html?lang=en)を参照してください。

>[!MORELIKETHIS]
>
>* [&#x200B; データの取り込み](https://experienceleague.adobe.com/ja/docs/experience-platform/ingestion/home)
>* [スキーマ](schemas.md)
>* [&#x200B; データ取り込みの概要（Experience Platform） &#x200B;](https://experienceleague.adobe.com/ja/docs/experience-platform/ingestion/home)
>* [Source コネクタ （Experience Platform） &#x200B;](https://experienceleague.adobe.com/en/docs/experience-platform/sources/home)
